...
export class DashexcComponent implements AfterViewInit { 
  ...
  makeDoughnut(id: string, canvasRef: any, labels: any, data: any, colors: any) { 
    ...
    this.charts[id] = new Chart(canvasRef, { 
      type:'doughnut', 
      data:{ labels, datasets:[{ data, backgroundColor:colors, borderColor:'#171e27', borderWidth:3 }] }, 
  ...
  parseMakand(wb: any) { 
    ...
    const month = this.pickLatestCompleteMonth(counts); 
    ...
    // ✅ Cambiado Object.values por Object.keys para corregir TypeScript
    const overallLlegada = total ? (Object.keys(transpMap).reduce((s: number, k: string) => s + (transpMap[k].llegadaOk || 0), 0) / total * 100) : 0; 
    const totalCajas = Object.keys(transpMap).reduce((s: any, k: string) => s + transpMap[k].cajas, 0); 
  ...
  // ✅ NUEVO METODO: renderAgri (Faltaba por completo en tu archivo)
